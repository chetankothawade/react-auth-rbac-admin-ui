...
const Header = ({ onChangeLayoutMode, layoutMode, headerClass }) => {
  ...
  const onLogout = useCallback(async () => {
    try {
      const response = await http.post("/client/logout", { role: authType });
            ...
            <div className="dropdown ms-sm-3 header-item topbar-user" id="page-header-user-dropdown">
              <Dropdown align="end">
                ...
                <Dropdown.Menu>
                  ...
                  {authUuid !== null && (
                    <Dropdown.Item as={Link} to={`/client/profile/${authUuid}`}>
                      <i className="mdi mdi-account-child-circle text-muted fs-16 me-1" />
                      My Company Profile
                    </Dropdown.Item>
                  )}
